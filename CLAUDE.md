...
- `config.js` — constants only (storage keys, `API_BASE`, `OAUTH_SCOPE`, states, tunables), each declared **exactly once** for the whole app; no module redeclares one locally. It is `queue.js`'s only import, so it must stay free of browser globals too or the Node tests break.
