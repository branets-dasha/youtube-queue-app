...
Per-channel prefs (`yqa_channel_prefs`, edited on `channels.html`) are read **fresh inside `runRefresh`** — never cached at startup — so edits made in a Channels tab apply to the next fetch without reloading the main tab. Ignored channels are skipped entirely in the per-channel loop (no uploads request at all — saves quota; their existing records are untouched). A channel `rate` preselects `preferredRate` on incoming records; only newly-inserted ones keep it, because the upsert never copies it onto an existing record — a refresh never changes a stored video's rate. Known limit (accepted): after un-ignoring a channel, "Fetch new" won't backfill videos published while it was ignored — use "Refresh all".
