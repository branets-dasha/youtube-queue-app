...
- `app.js` — the wiring/state hub (`state` object + `dom` refs); the only module that reaches into every layer. Owns event binding, all keyboard shortcuts, onboarding, `cleanup()`, `setVideoState`/`toggleSkip`, refresh orchestration.
