...
- **Single-tab guard** (`app.js` only): one named Web Lock (`yqa_tab`), so only one `index.html` tab runs — the newly-opened tab yields, the holder is never interrupted. `init()` requests it `ifAvailable` at the top and awaits the answer at its checkpoint, immediately before `cleanup()` (the first write path); not granted → `standDownForOtherTab()` + halt behind the shared `#blocked-overlay`. The grant is held for the document's lifetime by a callback promise that never settles (so `request()` never settles either — the answer comes out through a separate promise), and the browser releases it when the document dies. Race-free by construction: no handshake, no simultaneous-open window, and a frozen background tab still holds its lock. The store flag, not the overlay, is the real safety net: `bindEvents()` runs long before the checkpoint, so shortcuts stay live behind it and every write must actually throw (optimistic updates then revert). Decided once at startup — no mid-session halt, nothing to do about the player. `channels.html` never asks for the lock (it writes no video records, so it may be open alongside). **Fails open, never closed:** without `navigator.locks`, or if the request throws, the tab boots normally — the guard must never lock the owner out of their own queue.
