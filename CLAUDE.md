...
A channel `rate` is applied by `applyChannelRates(records, prefs, onlyVideoIds)` inside `mergeAndPersist`, always **after** the upsert (so brand-new arrivals are included — swapping that order would silently stop rating them). `queue.test.mjs` imports only `queue.js`, so its `mergeWithRates` helper locks a *mirror* of that composition, not the real one: the ordering in `app.js` and the `sweepRates` boolean at the two call sites are unguarded by the suite and must be kept correct by hand. The rule is **fill-if-absent**: a record whose `preferredRate` is unset (`undefined` *or* `null`) and whose channel has a rate gets it; a record with an explicit per-video rate is **never** overwritten or cleared; ignored channels are excluded.
