...
- `store.js` — video records in **IndexedDB** (`yqa` db, `videos` store, keyPath `videoId`), with a transparent localStorage fallback. Settings/floor/cutoff/channels/rates all in localStorage. `migrateStates()` runs on every read (normalizes any legacy non-`new` state → `skipped`).
